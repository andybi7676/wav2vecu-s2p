...
from ..data import ExtractedFeaturesDataset, RandomInputDataset, ExtractFeaturesDirectlyDataset
...
class UnpairedAudioText(FairseqTask):
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, directly_extract = False, directly = None, **kwargs):
        ...
        if directly_extract:
            assert directly != None, "Please add directly config!"
            directly_cfg = DirectlyExtractConfig(**directly)
            # directly_cfg.checkpoint = directly.checkpoint
            logger.info(f"[Directly Extract Config] = {directly_cfg}")
            self.datasets[split] = ExtractFeaturesDirectlyDataset(
                path=data_path,
                split=split,
                min_length=3,
