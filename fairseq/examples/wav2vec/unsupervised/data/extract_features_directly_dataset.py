...
class ExtractFeaturesDirectlyDataset(FairseqDataset):
    def __init__(
        ...
        self.pca_A = torch.from_numpy(np.load(self.pca_path + "_A.npy")).cuda()
        self.pca_b = torch.from_numpy(np.load(self.pca_path + "_b.npy")).cuda()
    ...
    def __getitem__(self, index):
        ...
        _, z = self.faiss_index.search(f, 1)
        print(z.shape)
        with torch.no_grad():
            x = torch.from_numpy(f).cuda()
            x = torch.matmul(x, self.pca_A) + self.pca_b
        print(x.shape)
...
if __name__ == '__main__':
    path = '/work/b07502072/corpus/u-s2s/audio/en_feats/voxpopuli/large_noisy'
    ...
    feats = [dataset.__getitem__(i) for i in range(3)]
    print(feats)
    ...
    feats_from_load = [from_load_dataset.__getitem__(i) for i in range(3)]
