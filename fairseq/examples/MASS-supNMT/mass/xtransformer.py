...
class XTransformerModel(BaseFairseqModel):
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, src_key, tgt_key):
        encoder_out = self.encoders[src_key](src_tokens, src_lengths)
        decoder_out = self.decoders[tgt_key](prev_output_tokens, encoder_out)
    ...
    @classmethod
    def build_model(cls, raw_args, task):
        args = task.args
        raw_args_dict = vars(raw_args)
        for key, val in raw_args_dict.items():
