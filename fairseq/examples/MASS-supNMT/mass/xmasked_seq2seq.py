...
from .asr_noise_language_pair_dataset import AsrNoiseLanguagePairDataset
...
def _get_asrmt_dataset_key(lang_pair):
    return "asrmt:" + lang_pair
...
class XMassTranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--asrmt_steps', default='', metavar='LANG-PAIRS',
                            help='Mimic asr input for machine translation')
        ...
        parser.add_argument('--ins_rate', default=0.05, type=float, metavar='RATIO',
                            help='The mask ratio')
        parser.add_argument('--del_rate', default=0.05, type=float, metavar='RATIO',
                            help='The mask ratio')
        parser.add_argument('--sub_rate', default=0.10, type=float, metavar='RATIO',
                            help='The mask ratio')
        parser.add_argument('--word_mask', default=0.25, type=float, metavar='RATIO',
                            help='The mask ratio')
        parser.add_argument('--use_vocab_weights', default=True, type=bool,
                            help='Use vocab weights as insertion sampling proability')
    ...
    def setup_task(cls, args, **kwargs):
        args, dicts, training = cls.prepare(args, **kwargs)
    ...
    def prepare(cls, args, **kwargs):
        ...
        args.asrmt_steps = [s for s in args.asrmt_steps.split(',') if len(s) > 0]
        ...
        args.para_lang_pairs = list(set([
            lang_pair
            for lang_pair in set(args.mt_steps + args.memt_steps + args.asrmt_steps) if 
            len(lang_pair) > 0
        ]))
        ...
        for lang_pair in args.mt_steps + args.memt_steps + args.asrmt_steps:
            src, tgt = lang_pair.split('-')
            assert src in args.source_langs and tgt in args.target_langs
        ...
        if args.source_lang is not None and args.target_lang is not None:
            ...
            if len(args.para_lang_pairs) > 0:
                required_para = [s for s in set(args.mt_steps + args.memt_steps + args.asrmt_steps)]
    ...
    def load_dataset(self, split, **kwargs):
        ...
        def split_exists(split, lang):
            ...
            if self.args.raw_text and IndexedRawTextDataset.exists(filename):
                return True
            elif not self.args.raw_text and MMapIndexedDataset.exists(filename):
                return True
            return False
        ...
        for lang_pair in self.args.mt_steps:
            ...
            mt_para_dataset[lang_pair] = LanguagePairDataset(
                ...
                tgt=tgt_dataset, tgt_sizes=tgt_dataset.sizes, tgt_dict=self.dicts[tgt],
        ...
        asrmt_para_dataset = {}
        if split == 'train':
            for lang_pair in self.args.asrmt_steps:
                src, tgt = lang_pair.split('-')
                key = '-'.join([src, tgt])
                src_key = key + '.' + src
                tgt_key = key + '.' + tgt
                ...
                src_dataset = src_para_datasets[src_key]
                tgt_dataset = src_para_datasets[tgt_key]
                asrmt_para_dataset[lang_pair] = AsrNoiseLanguagePairDataset(
                    src_dataset, src_dataset.sizes, self.dicts[src],
                    tgt=tgt_dataset, tgt_sizes=tgt_dataset.sizes, tgt_dict=self.dicts[tgt],
                    left_pad_source=self.args.left_pad_source,
                    left_pad_target=self.args.left_pad_target,
                    # max_source_positions=self.args.max_source_positions,
                    # max_target_positions=self.args.max_target_positions,
                    ins_rate=self.args.ins_rate,
                    del_rate=self.args.del_rate,
                    sub_rate=self.args.sub_rate,
                    mask_ratio=self.args.word_mask,
                    pred_probs=self.args.pred_probs,
                )
        ...
        self.datasets[split] = RoundRobinZipDatasets(
            OrderedDict([
                ...
                (_get_asrmt_dataset_key(lang_pair), asrmt_para_dataset[lang_pair])
                for lang_pair in asrmt_para_dataset.keys()
            ] + [
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False):
        ...
        agg_loss, agg_sample_size, logging_outputs = 0., 0., []
        for lang_pair in self.args.mt_steps:
            sample_key = lang_pair
            loss_mt, sample_size_mt, log_mt = forward_backward(model, sample[sample_key], sample_key, lang_pair)
            agg_loss += loss_mt
            agg_sample_size += sample_size_mt
            logging_outputs.append(log_mt)

        for lang_pair in self.args.memt_steps:
            sample_key = _get_memt_dataset_key(lang_pair)
            loss_memt, sample_size_memt, log_memt = forward_backward(model, sample[sample_key], sample_key, lang_pair)
            agg_loss += loss_memt
            agg_sample_size += sample_size_memt
            logging_outputs.append(log_memt)
        
        for lang_pair in self.args.asrmt_steps:
            sample_key = _get_asrmt_dataset_key(lang_pair)
            loss_asrmt, sample_size_asrmt, log_asrmt = forward_backward(model, sample[sample_key], sample_key, lang_pair)
            agg_loss += loss_asrmt
            agg_sample_size += sample_size_asrmt
            logging_outputs.append(log_asrmt)

        for lang_pair in self.args.mass_steps:
            sample_key = _get_mass_dataset_key(lang_pair)
            loss_mass, sample_size_mass, log_mass = forward_backward(model, sample[sample_key], sample_key, lang_pair)
            agg_loss += loss_mass
            agg_sample_size += sample_size_mass
            logging_outputs.append(log_mass)
        # agg_logging_output = criterion.reduce_metrics(logging_outputs)
        agg_logging_output = self._aggregate_logging_outputs(logging_outputs)
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            ...
            for lang_pair in self.args.valid_lang_pairs:
                ...
                # print(agg_loss, agg_sample_size, logging_output)
        agg_logging_output = self._aggregate_logging_outputs(logging_outputs)
        return agg_loss, agg_sample_size, agg_logging_output
    ...
    def _aggregate_logging_outputs(self, logging_outputs):
        if len(logging_outputs) < 1: return None
        final_logging_output = logging_outputs[0]
        for log_output in logging_outputs[1:]:
            for key, val in log_output.items():
                final_logging_output[key] += val
        return final_logging_output
