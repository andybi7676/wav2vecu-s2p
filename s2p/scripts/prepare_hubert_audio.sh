...
# --checkpoint $model --save-dir $tgt_dir -f CLUS$k --sample-pct 1.0
# echo "Finished clustering."

# for split in $all_splits; do
#   python scripts/wav2vec_apply_cluster_faiss.py $tgt_dir \
#   --checkpoint $model --path $tgt_dir/CLUS$k --split $split
# done
...
for split in $all_splits; do
  ...
  python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/merge_clusters.py $tgt_dir/precompute_pca$dim --cluster-dir $tgt_dir/CLUS$k \
  --split $split --save-dir $tgt_dir/precompute_pca${dim}_cls${k}_mean --pooling mean

  python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/mean_pool.py $tgt_dir/precompute_pca${dim}_cls${k}_mean \
  --save-dir $tgt_dir/precompute_pca${dim}_cls${k}_mean_pooled --split $split
done
