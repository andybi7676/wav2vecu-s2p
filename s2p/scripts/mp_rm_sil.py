#!/usr/bin/env python3 -u
# Copyright (c) Facebook, Inc. and its affiliates.
#
# This source code is licensed under the MIT license found in the
# LICENSE file in the root directory of this source tree.

"""
get intervals from .vads file, specify output data, and this script removes silences and saves the audio data in out path folder
paths=shards/train.tsv
vads=shards/train.vads
python remove_silence.py --paths $paths --vads $vads
"""

import os
import os.path as osp
import argparse
import torch
import torchaudio
import tqdm
import librosa as lb
from functools import partial
import multiprocessing as mp
SAMPLE_RATE = 16_000

parser = argparse.ArgumentParser()
parser.add_argument("--tsv", default="", type=str)
parser.add_argument("--vads", default="", type=str)
parser.add_argument("--out", type=str)
parser.add_argument("--overwrite", default=False, action="store_true")
params = parser.parse_args()

# load paths
paths = []
with open(params.tsv) as f:
    root = next(f).rstrip()
    for line in f:
        paths.append(os.path.join(root, line.rstrip().split("\t")[0]))

# load vads
list_intervals = []
with open(params.vads) as f:
    for line in f:
        interval = [
            [int(w.split(":")[0]), int(w.split(":")[1])] for w in line.rstrip().split()
        ]
        list_intervals.append(interval)


# load audio and keep only intervals (i.e. remove silences)

def rm_sil(args):
    path, list_interval, params = args
    if not params.overwrite:
        sub_path = osp.relpath(path, params.root)
        sub_path = sub_path.split('.')[0] + ".wav"
        outpath = osp.join(params.out, sub_path)
        if os.path.exists(outpath): return True
    data, sr = lb.load(path, sr=SAMPLE_RATE)
    assert sr == SAMPLE_RATE
    wav = torch.FloatTensor([data])
    if len(list_interval) > 0:
        data_filtered = torch.cat(
            [wav[0][int(it[0]) : int(it[1])] for it in list_interval]
        ).unsqueeze(0)
    else:
        data_filtered = wav

    # YOU MAY NEED TO MODIFY THIS TO GET THE RIGHT SUBPATH
    # outpath = params.out + '/' + '/'.join(paths[i].split('/')[-1:])
    sub_path = osp.relpath(path, params.root)
    sub_path = sub_path.split('.')[0] + ".wav"
    outpath = osp.join(params.out, sub_path)
    # outpath = params.out + "/" + "/".join(paths[i].split("/")[-2:])
    # print(outpath)
    if not os.path.isdir("/".join(outpath.split("/")[:-1])):
        os.makedirs("/".join(outpath.split("/")[:-1]), exist_ok=True)
    if not os.path.exists(outpath):
        torchaudio.save(outpath, data_filtered, sample_rate=SAMPLE_RATE)
    else:
        # raise Exception
        if params.overwrite:
            print(outpath, "exists and overwrite!")
            torchaudio.save(outpath, data_filtered, sample_rate=SAMPLE_RATE)
        else:
            print(outpath, "exists and skip!")
    return True

params.root = root
params_list = [params] * len(paths)
pool = mp.Pool()
success_list = list(tqdm.tqdm(pool.imap(rm_sil, list(zip(paths, list_intervals, params_list))), total=len(paths)))
print(sum(success_list) == len(paths))

