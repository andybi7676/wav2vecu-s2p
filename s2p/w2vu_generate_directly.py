...
from examples.speech_recognition.kaldi.kaldi_initializer import KaldiInitializerConfig, initalize_kaldi
...
def main(cfg: UnsupGenerateConfig, model=None):
    ...
    logger.info(res)
    with open(os.path.join(cfg.results_path, f"WER={wer}.res"), 'w') as fw:
        fw.write(res)
    # print(res)
...
@hydra.main(
    config_path=os.path.join("/home/andybi7676/Desktop/wav2vecu-s2p/fairseq", "fairseq", "config"), config_name="config"
)
def hydra_main(cfg):
    with open_dict(cfg):
